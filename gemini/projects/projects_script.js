...
function filterProjects() {
    console.log('Filtering projects...'); // Debug log
...
function renderProjects() {
    ...
    // Add error checking with more detailed logging
    if (!workGrid) {
        console.error('workExperienceGrid element not found');
        return;
    }
    if (!projectsGrid) {
        console.error('personalProjectsGrid element not found');
        return;
    }
    if (!noResults) {
        console.error('noResults element not found');
        return;
    }
    ...
    console.log('Work projects:', workProjects.length, 'Personal projects:', personalProjects.length);

    if (workProjects.length === 0 && personalProjects.length === 0) {
        noResults.style.display = 'block';
        document.getElementById('workExperienceContainer').style.display = 'none';
        document.getElementById('personalProjectsContainer').style.display = 'none';
    } else {
        noResults.style.display = 'none';
        document.getElementById('workExperienceContainer').style.display = workProjects.length > 0 ? 'block' : 'none';
        document.getElementById('personalProjectsContainer').style.display = personalProjects.length > 0 ? 'block' : 'none';
    }
        ...
        <div class="project-card" data-project-id="${project.id}">
            <div class="project-content">
                ...
                <div class="project-links">
                    ...
                    ${project.demo ? `<a href="${project.demo}" target="_blank" class="project-link" onclick="event.stopPropagation()">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (positionFilter) {
        positionFilter.addEventListener('change', (e) => {
            ...
            console.log('Position filter changed to:', activePositionFilter);
            filterProjects();
        });
    }
    ...
    if (categoryFilter) {
        categoryFilter.addEventListener('change', (e) => {
            ...
            console.log('Category filter changed to:', activeCategoryFilter);
            filterProjects();
        });
    }
